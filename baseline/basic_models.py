...
def create_preprocess_function(tokenizer, max_length: int = 100, padding_max_length: bool = True,
                               with_cls: bool = False):
...
def extract_embeddings(input_ids: List[List[int]], model: PreTrainedModel,
                       batch_size: int = None, mean_embeddings: bool = False, device: str = 'cuda') -> List[List[float]]:
    # device = model.device
    model = model.eval()
    try:
        embeddings = model.embed_layer.to(device) #model.get_input_embeddings()
    except:
        try:
            embeddings = model.word_emb_layer.to(device)
        except:
            embeddings = model.emb_layer.to(device)
