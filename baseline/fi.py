...
def extract_gradients(model: Module,
    ...
    if modality == 'text':
        ...
        if isinstance(model, PreTrainedModel):
            ...
            try:
                _, (h, c) = model.gru(inputs)
                g = torch.cat((h, c), dim=-1)
                outputs = model.out_layer(g) 
            except:
                try:
                    pe = model.position_emb_layer(inputs)
                    attended = model.transformer_layer(pe) # [B, L, D]
                    _, (h, _) = model.mid_layer(attended)
                    out = torch.cat((h[0, :], h[1, :]), dim=-1)
                    outputs = model.out_layer(out)
                except:
                    output = model.encoder(inputs)
                    output = model.drop(output)
                    outputs = model.out(output)
